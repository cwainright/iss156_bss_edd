...
options(warn=-1)
bob_2022_habitat_results <- function(results_list, bob_2022_hab, example){
    tryCatch(
        expr = {
            ...
            source("modules/bob/bob_2022_hab_unit_lookup.R")
            
            # make a flat dataframe from `results_list`
            df <- bob_2022_hab %>%
                subset(`Site` %like% "NCRN") %>%
                select(-c(Locality, Crew, `Stream Name`))
            id_vars <- c("Site", "Date sampled", "Time sampled") # the column(s) we want to keep as columns
            ...
            df$`Time sampled` <- format(df$`Time sampled`, "%H:%M")
            # make required EDD fields
            df$Characteristic_Name <- "Stream habitat inventory"
            df$Activity_ID <- paste0(df$`Site`, "_", df$`Date sampled`, "_", df$`Time sampled`, "_", df$variable)
            df$Activity_ID <- gsub(" ", "_", df$Activity_ID)
            # add NCRN fields
            ...
            unit_lookup <- bob_2022_hab_unit_lookup()
            ...
            real[2] <- paste0(df$Site, ".b.", as.character(format(df$`Date sampled`, "%Y%m%d"))) # "Activity_ID" shared field with `real_activities.Activity_ID`
            ...
            real[35] <- format(df$`Date sampled`, "%Y-%m-%d") # "Analysis_Start_Date"
            real[36] <- df$`Time sampled` # "Analysis_Start_Time" 
            ...
            real[89] <- "nps_spring_habitat_2022_bob.xlsx" # "Result_File_Name"
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`bob_2022_habitat_results()` executed successfully..."
                } else {
