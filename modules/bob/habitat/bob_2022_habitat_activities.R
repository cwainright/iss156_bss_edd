...
options(warn=-1)
bob_2022_habitat_activities <- function(results_list, bob_2022_hab, example){
    tryCatch(
        expr = {
            ...
            df <- bob_2022_hab %>% select(`Site`, `Date sampled`, `Time sampled`, `Stream Name`, `Locality`, Crew, Benthos)
            ...
            real[9] <- format(df$`Date sampled`, "%Y-%m-%d") # "Activity_Start_Date"
            real[10] <- format(df$`Time sampled`, "%H:%M") # "Activity_Start_Time" 
            ...
            real[21] <- df$`Stream Name` # "Additional_Location_Info"
            real[22] <- NA # "Activity_Sampler"; the person who did the sampling?
            real[23] <- df$Crew # "Activity_Recorder"
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`bob_2022_habitat_activities()` executed successfully..."
                } else {
