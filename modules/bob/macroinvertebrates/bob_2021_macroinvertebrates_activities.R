...
options(warn=-1)
bob_2021_macroinvert_activities <- function(results_list, bob_2021_water_chem, example){
    tryCatch(
        expr = {
            ...
            real[4] <- paste0(df$NCRN_Site_ID, ".b.", format(df$`Date Collected`, "%Y%m%d")) # "Activity_ID" shared field with `real_locations.Activity_ID` and `real_results.Activity_ID`
            ...
            real[41] <- df$`Sample ID` # "Activity_Group_Name" 
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`bob_2021_macroinvert_activities()` executed successfully..."
                } else {
