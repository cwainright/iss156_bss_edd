...
options(warn=-1)
bob_2021_macroinvert_results <- function(results_list, bob_2021_macroinvert, bob_2021_water_chem, example){
    tryCatch(
        expr = {
            ...
            df <- dplyr::left_join(df, loc_lookup, by=c("site" = "bob_site"))
            
            lookup <- results_list$tlu_Macroinverts
            df <- dplyr::left_join(df, lookup, by=c("Genus" = "Taxon"))
            df$Taxon_ID <- tolower(df$Genus)
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`bob_2021_macroinvert_results()` executed successfully..."
                } else {
