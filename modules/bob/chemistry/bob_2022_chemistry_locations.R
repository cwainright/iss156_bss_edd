...
options(warn=-1)
bob_2022_chemistry_locations <- function(results_list, bob_2022_wq, example){
    tryCatch(
        expr = {
            ...
            df <- bob_2022_wq %>%
                select(`Sample ID`) %>% # we ony want location IDs
                subset(`Sample ID` %like% "NCRN") # we only want site names, sometimes there are reference values here
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`bob_2022_chemistry_locations()` executed successfully..."
                } else {
