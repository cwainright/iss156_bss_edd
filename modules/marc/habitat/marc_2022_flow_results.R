...
# ETL summer_flow_marc2022 into edd.results
options(warn = -1)
marc_2022_flow_results <- function(summer_flow_marc2022, example, results_list){
    tryCatch(
        expr = {
            ...
            real[2] <- paste0(df$NCRN_Site_ID, ".m.", format(as.Date(df$Date), "%Y%m%d")) # "Activity_ID"
            ...
            real[35] <- format(as.Date(df$Date), "%Y-%m-%d") # "Analysis_Start_Date"
            ...
            real[89] <- "'ncrn_bss_fish_monitoring_data_stream_habitat_2022_marc.xlsx' sheet 'Summer Habitat Flow'" # "Result_File_Name"
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`marc_2022_flow_results()` executed successfully..."
                } else {
