...
options(warn=-1)
ncrn_macroinvert_results <- function(results_list, example){
    tryCatch(
        expr = {
            ...
            df <- results_list$tbl_Benthic_Data
            df$Characteristic_Name <- "stream macroinvertebrate sampling"
            df$Result_Text <- df$N_TAXA
            df$Result_Unit <- "count of individuals"
            # join taxonomic values
            lookup <- results_list$tlu_Macroinverts
            df <- dplyr::left_join(df, lookup, by=c("Taxon_ID" = "Taxon"))
            df$Taxon_ID <- tolower(df$Taxon_ID)
            ...
            real[12] <- df$Comment # "Result_Comment" 
            ...
            real[50] <- tolower(df$TAXON) # "Subject_Taxon"
            ...
            real[89] <- "tbl_Benthic_Data" # "Result_File_Name"
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`ncrn_macroinvert_results()` executed successfully..."
                } else {
