...
options(warn=-1)
ncrn_habitat_activities <- function(results_list, example){
    tryCatch(
        expr = {
            ...
            # df_spring <- dplyr::left_join(df_spring, results_list$tbl_Events %>% select(Event_ID, Protocol_ID, Start_Date, Start_Time, Location_ID, Comments, Event_Site_ID, Event_Group_ID), by = c("PHI_ID" = "Event_ID"))
            ...
            # df_summer <- dplyr::left_join(df_summer, results_list$tbl_Events %>% select(Event_ID, Protocol_ID, Start_Date, Start_Time, Location_ID, Comments, Event_Site_ID, Event_Group_ID), by = c("PHI_ID" = "Event_ID"))
            ...
            message(
                if(length(check_df$result == "MATCH") == nrow(check_df)){
                    "`ncrn_habitat_activities()` executed successfully..."
                } else {
